...
// swap two values
void Eigenvalue::swap(double &a, double &b) {
	double temp;
	temp = a;
	a = b;
	b = temp;
...
//change matrix to array for later use
void Eigenvalue::MatrixToArray(Matrix a, double * A) {
	double Atemp[MAX_N][MAX_N];
	bool t;
	int m, n,i, j;
	m = a.getCol();
	n = a.getRow();
...
void Eigenvalue::MatrixHessenberg(double * A, int n, double * result) {
	int max;
	int k, i, j;
	double temp;
...
bool Eigenvalue::MatrixEigenValue(Matrix a, int n, int LoopNumber,double *result) {
